...
We will use the `caret` package in R to train a SVM classifier to do text classification.

First we will do some data pre-processing.
...
spam.train$label<-0
spam.train$label = ifelse(spam.train$type=="ham",0,1)

corpus_msg <- Corpus(VectorSource(spam.train[,2]))

tdm_msg <- DocumentTermMatrix(corpus_msg, list(removePunctuation = TRUE, 
                                               removeNumbers = TRUE))

training_set_msg <- as.matrix(tdm_msg)

#adding the class label column
training_set_msg <- cbind(training_set_msg, spam.train$label)

#assigning last column's name y
colnames(training_set_msg)[ncol(training_set_msg)] <- "y"

#converting the DFM matrix to a large data frame
training_set_msg <- as.data.frame(training_set_msg)
training_set_msg$y <- as.factor(training_set_msg$y)
```
...
svmMod1<-train(spam.train[,1] ~ .,data = msg.dfm.train,method = 'svmLinear3')
