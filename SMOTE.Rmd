...
###Let's start off the implementation in R

```{r}
require(DMwR)
table(spam$type)
spam1<-spam
spam1$type<-ifelse(spam1$type=="spam",1,2)#if type=spam , then 1, otherwise 2
spam1$type<-as.factor(spam1$type)  

newSpam<-SMOTE(type ~., data=spam1,perc.over =100, perc.under = 200 )
